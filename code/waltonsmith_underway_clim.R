...
tmps <- list()
png('WS_underway_summer_clim_maps2.png',width = 9, height = 14, res=300, units='in')
...
setwd('~/Desktop/professional/projects/Postdoc_FL/figures/')
png('WS_underway_summer_clim2.png',width = 8, height = 10, res=300, units='in')
...
mtext('Walton Smith underway data')
dev.off()
...
hists <- data.frame(matrix(NA,length(ind),length(brks)-1))
tmps <- list()
png('WS_underway_summer_clim_maps_fk2.png',width = 9, height = 9, res=300, units='in')
par(mar=c(2,2,2,2),mfrow=c(3,2))
for(i in ind){
  setwd('~/Desktop/professional/projects/Postdoc_FL/data/walton_smith/underway_data')
  data <- read.csv(fil[i])
...
setwd('~/Desktop/professional/projects/Postdoc_FL/figures/')
png('WS_underway_summer_clim_fk2.png',width = 8, height = 10, res=300, units='in')
par(mar=c(5,5,2,2),mfrow=c(1,1))
...
segments(median(tmps,na.rm=T),rep(at[length(ind)+1]-.03,15),median(tmps,na.rm=T),rep(at[length(ind)+1]-.21,15),col='gold',lwd=2,lend=2)
axis(2,at[length(ind)+1],'Overall',las=2)
mtext('Walton Smith underway data - FKNMS')
dev.off()
...
hists <- data.frame(matrix(NA,length(ind),length(brks)-1))
tmps <- list()
png('WS_underway_summer_clim_maps_swfl2.png',width = 9, height = 14, res=300, units='in')
par(mar=c(2,2,2,2),mfrow=c(3,2))
for(i in ind){
  setwd('~/Desktop/professional/projects/Postdoc_FL/data/walton_smith/underway_data')
  data <- read.csv(fil[i])
...
setwd('~/Desktop/professional/projects/Postdoc_FL/figures/')
png('WS_underway_summer_clim_swfl2.png',width = 8, height = 10, res=300, units='in')
par(mar=c(5,5,2,2),mfrow=c(1,1))
...
segments(median(tmps,na.rm=T),rep(at[length(ind)+1]-.03,15),median(tmps,na.rm=T),rep(at[length(ind)+1]-.21,15),col='gold',lwd=2,lend=2)
axis(2,at[length(ind)+1],'Overall',las=2)
mtext('Walton Smith underway data - SWFL')
dev.off()
